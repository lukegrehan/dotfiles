alias gain='mp3gain -r *.mp3'
alias du='du -kh'
alias df='df -kh'
alias mkdir='mkdir -p -v'
alias rm='rm -r'
alias cdtemp='cd $(mktemp --dir)'
alias pushtemp='pushd . && cdtemp'
alias chalias='$EDITOR ~/.bash_aliases'
alias title='mocp -Q%title'
alias t='tree --filesfirst'
alias shutdown='shutdown now'
alias mntPi='mkdir -p ~/Pi && sshfs luke@pi:/home/luke ~/Pi -o idmap=user && cd ~/Pi'
alias unMntPi='cd && fusermount -u ~/Pi && rmdir ~/Pi'
alias mntPhone='mkdir -p ~/Phone && simple-mtpfs ~/Phone && cd ~/Phone'
alias unMntPhone='cd && fusermount -u ~/Phone && rmdir ~/Phone'
alias tmux='tmux -2'
alias v='vim'
alias ytdl='youtube-dl -f bestaudio --extract-audio --audio-format mp3'
alias startRemote='/opt/urserver/urserver --daemon'
alias pandoc='pandoc --pdf-engine=xelatex'
alias pacman='paru --sudoloop'
alias ag='ag --pager="less -rFi"'
alias incog=' exec bash --init-file <(echo ". ~/.bashrc; unset HISTFILE")'
alias pyhton='python'
alias youtube-dl='yt-dlp'
alias neofetch='fastfetch'
alias mocp='cowsay "use mus"'

